    ...
    for i in range(len(column_dicts)):
        ...
        for key in column_dict.keys():
            columns.append(pyfits.Column(
                name=key, format="D", array=column_dict[key]))
...
def fit(fitter, header, orders, gdas_filename="GDAS_Atmosphere.dat", outfilename='Corrected.fits'):
    # Read in the appropriate parameters:
    angle = numpy.abs(float(header['AZ']))  # TODO check this
    resolution = 60000.0
    humidity = header['RELHUM'] * 100 #to Percent
    pressure = header['AIRPRESS'] * 1000 # to hectoPascal
    temperature = header['TEMPOUT'] + 273.15  # Convert to Kelvin

    # Adjust the atmosphere profile using GDAS data
    height, Pres, Temp, h2o = GetProfile(gdas_filename)
    ...
    for i in range(len(orders)):
        ...
        order.cont = FittingUtilities.Continuum(
            order.x, order.y, fitorder=3, lowreject=1.5, highreject=10)
    ...
    humidity = numpy.sum(numpy.array(
        h2o) * numpy.array(weights)) / numpy.sum(weights)
    # This turns h2o off as a fitted variable!
    ...
    velshifts = waveshifts / wave0 * \
        constants.c.cgs.value * units.cm.to(units.km)
    ...
    for i, order in enumerate(orders):
        ...
        fitpars = [fitter.const_pars[j]
                   for j in range(len(fitter.parnames)) if fitter.fitting[j]]
        ...
        if min(model.y) > 0.98:
            wave0 = order.x.mean()
            fitter.shift = vel / (constants.c.cgs.value *
                                  units.cm.to(units.km)) * wave0
            model = fitter.GenerateModel(
                fitpars, separate_source=False, nofit=True)
            model.x /= (1.0 + vel / (constants.c.cgs.value *
                                     units.cm.to(units.km)))
            model = FittingUtilities.RebinData(model, order.x)
            data = order.copy()
            data.cont = FittingUtilities.Continuum(
                data.x, data.y, fitorder=3, lowreject=2, highreject=5)
        ...
        if i == 0:
            OutputFitsFileExtensions(columns, fname, outfilename, headers_info=[
                                     header_info, ], mode="new")
        else:
            OutputFitsFileExtensions(columns, outfilename, outfilename, headers_info=[
                                     header_info, ], mode="append")
