...
def load_intensities(config, par, wl_flux, flux):
    # Limb darkening
    ...
    T = round_to(par['star_temp'], 250, limits=[3500, 4500])
    logg = round_to(par['star_logg'], 0.5, limits=[0, 5])
    vmt = round_to(par['star_vt'], 1, limits=[0, 8])
    met = round_to(par['star_metallicity'], 0.1, limits=[-5, 0.5])
    if met == 0.4:
        met = 0.3
