...
class PlanetParametersStep(Step, StepIO):
    ...
    def run(self, spectra, telluric, star, planet):
        planet = self.extract_transit_parameters(spectra, telluric, star, planet)
        self.save(planet)
        return planet
    ...
    def lnlike(self, theta, x, y):
        m = self.model(x, theta)
        return -0.5 * np.sum((y - m) ** 2)

    def lnprob(self, theta, x, y, trange):
    ...
    def first_guess(self, flux, times, star, planet):
    ...
    def run_mcmc(self, times, flux, p0):
    ...
    def extract_transit_parameters(self, spectra, telluric, star, planet):
        names = ["t0", "per", "$R_p$", "a", "inc", "ecc", "w", "u1", "u2", "cont"]
        per = planet.period.to_value("day")

        print("Create lightcurve")
        times = spectra.datetime
        times, flux = self.normalize_spectra(spectra, telluric, times)

        print("Fit lightcurve with least-squares")
        popt = self.first_guess(flux, times, star, planet)
