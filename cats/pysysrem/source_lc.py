...
class source:
    ...
    def remove_epoch(self, epoch):
        if epoch in self.epochs:
            ...
            self.xschip = np.array(
                list(self.xschip[0:epoch_n]) + list(self.xschip[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.yschip = np.array(
                list(self.yschip[0:epoch_n]) + list(self.yschip[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.xs = np.array(
                list(self.xs[0:epoch_n]) + list(self.xs[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.ys = np.array(
                list(self.ys[0:epoch_n]) + list(self.ys[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.mags = np.array(
                list(self.mags[0:epoch_n]) + list(self.mags[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.magerrors = np.array(
                list(self.magerrors[0:epoch_n]) + list(self.magerrors[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.epochs = np.array(
                list(self.epochs[0:epoch_n]) + list(self.epochs[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.flags = np.array(
                list(self.flags[0:epoch_n]) + list(self.flags[epoch_n + 1 :]),
                dtype=np.float32,
            )
            self.images = np.array(
                list(self.images[0:epoch_n]) + list(self.images[epoch_n + 1 :])
            )
        else:
            print(
                f"Tried to remove epoch {epoch}, but not present in list {self.epochs}"
            )
